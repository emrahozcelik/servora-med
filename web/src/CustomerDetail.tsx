...
import {
  ...
  type CustomerDetail, type CustomerJobSummary, type CustomerType,
...
export function customerMutationErrorMessage(error: unknown) {
  ...
  if (error instanceof ApiError && error.code === 'VERSION_CONFLICT') return 'Müşteri başka bir kullanıcı tarafından güncellendi. Güncel sürüm yüklendi; formdaki değişiklikleriniz korunuyor.';
...
function CustomerEditForm({ customer, staff, pending, onSave }: { customer: CustomerDetail; staff: StaffProfile[]; pending: boolean; onSave: (event: FormEvent<HTMLFormElement>) => void }) {
...
export function CustomerDetailView({ customer, user, staff, pending, error, notice, errorRef, onBack, onSave, onLifecycle, onCreateContact }: {
  ...
  return <main className="customer-detail"><button className="back-link" type="button" onClick={onBack}>Müşterilere dön</button>
    ...
    <section className="record-section" aria-labelledby="general-title"><div className="section-heading"><h2 id="general-title">Genel bilgiler</h2><span>Sürüm {customer.version}</span></div>
      {canManage ? <CustomerEditForm customer={customer} staff={staff} pending={pending} onSave={onSave} /> : <CustomerFacts customer={customer} />}</section>
    {canManage && <section className="record-section record-commands" aria-labelledby="customer-status-title"><h2 id="customer-status-title">Müşteri durumu</h2><p>Durum değişikliği iş ve ilgili kişi oluşturma kurallarını etkiler.</p>
      <button className="secondary-button" type="button" disabled={pending} onClick={(event) => onLifecycle(customer.status === 'inactive' ? 'activate' : 'deactivate', event.currentTarget)}>
...
export function CustomerDetailScreen({ customerId, user }: { customerId: string; user: CurrentUser }) {
  ...
  const [creatingContact, setCreatingContact] = useState(false); const [contactError, setContactError] = useState(''); const errorRef = useRef<HTMLDivElement>(null);
  const load = () => { setLoading(true); setCustomer(null); setError(''); Promise.all([getCustomer(customerId), user.role === 'STAFF' ? Promise.resolve([]) : listStaff('active')])
    .then(([record, profiles]) => { setCustomer(record); setStaff(profiles); }).catch((caught) => setError(caught instanceof Error ? caught.message : 'Müşteri yüklenemedi.')).finally(() => setLoading(false)); };
  useEffect(load, [customerId, user.role]); useEffect(() => { if (error) errorRef.current?.focus(); }, [error]);
  if (loading) return <main className="customer-detail" aria-busy="true"><h1>Müşteri detayı yükleniyor</h1></main>;
  if (!customer) return <main className="customer-detail"><div className="workspace-message" role="alert"><h1>Müşteri yüklenemedi</h1><p>{error}</p><button className="secondary-button" onClick={load}>Tekrar dene</button></div></main>;
  async function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setPending(true); setError(''); setNotice('');
    try { const updated = await updateCustomer(customerId, customerFieldsFromFormData(new FormData(event.currentTarget), customer!.version)); setCustomer({ ...customer!, ...updated }); setNotice('Müşteri bilgileri güncellendi.'); }
    catch (caught) { if (caught instanceof ApiError && caught.code === 'VERSION_CONFLICT') await getCustomer(customerId).then(setCustomer).catch(() => undefined); setError(customerMutationErrorMessage(caught)); } finally { setPending(false); } }
  async function lifecycle(action: 'activate' | 'deactivate', trigger?: HTMLButtonElement) {
    if (!confirmCustomerLifecycle(customer!, action)) { trigger?.focus(); return; } setPending(true); setError('');
    try { const updated = action === 'activate' ? await activateCustomer(customerId, customer!.version) : await deactivateCustomer(customerId, customer!.version); setCustomer({ ...customer!, ...updated }); setNotice(action === 'activate' ? 'Müşteri aktifleştirildi.' : 'Müşteri pasifleştirildi.'); }
    catch (caught) { if (caught instanceof ApiError && caught.code === 'VERSION_CONFLICT') await getCustomer(customerId).then(setCustomer).catch(() => undefined); setError(customerMutationErrorMessage(caught)); } finally { setPending(false); trigger?.focus(); } }
  async function createContactRecord(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setPending(true); setContactError('');
    try { const created = await addContact(customerId, new FormData(event.currentTarget)); setCustomer({ ...customer!, contacts: [...customer!.contacts.map((contact) => created.isPrimary ? { ...contact, isPrimary: false } : contact), created], primaryContact: created.isPrimary ? { id: created.id, name: created.name, title: created.title } : customer!.primaryContact }); setCreatingContact(false); setNotice('İlgili kişi eklendi.'); }
    catch (caught) { setContactError(caught instanceof Error ? caught.message : 'İlgili kişi eklenemedi.'); } finally { setPending(false); } }
  return <><CustomerDetailView customer={customer} user={user} staff={staff} pending={pending} error={error} notice={notice} errorRef={errorRef}
