...
export function DeliveryReport({ user }: { user: CurrentUser }) {
  ...
  return <main className="workspace report-workspace"><header className="workspace-heading"><div><p className="eyebrow">Raporlar</p><h1>Teslim raporu</h1></div><nav className="report-nav"><Link to={paths.reports}>Özet</Link><Link to={paths.approvalReports}>Onaylar</Link></nav></header>
    <form key={JSON.stringify([state.from, state.to, state.groupBy, state.staffUserId])} className="report-filters report-filters-wide" onSubmit={submit}><label>Başlangıç<input name="from" type="date" defaultValue={state.from ?? ''} /></label><label>Bitiş<input name="to" type="date" defaultValue={state.to ?? ''} /></label>
      <label>Gruplama<select name="groupBy" defaultValue={state.groupBy}><option value="day">Gün</option><option value="purpose">Amaç</option><option value="product">Ürün</option><option value="staff">Personel</option></select></label>
      <label>Personel<select name="staffUserId" defaultValue={state.staffUserId ?? ''} disabled={options.status === 'loading'}><option value="">Tüm personel</option>{unavailable && <option value={state.staffUserId!}>Seçili personel (listede yok)</option>}{options.items.map((item) => <option key={item.user.id} value={item.user.id}>{item.user.name}{item.user.isActive ? '' : ' (Pasif)'}</option>)}</select></label>
    ...
    {formError && <p ref={errorRef} className="field-error" role="alert" tabIndex={-1}>{formError}</p>}
