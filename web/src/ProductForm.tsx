...
export function ProductForm({ pending, fieldErrors, error, errorRef, onCancel, onSubmit }: {
  pending: boolean;
  ...
  return <main className="product-create">
    <div className="detail-heading"><div><p className="eyebrow">Ürün kataloğu</p><h1>Yeni ürün</h1></div></div>
    <p className="form-intro">Katalog kaydını temel ürün bilgileriyle oluşturun. Yalnız ürün adı zorunludur.</p>
    ...
    <form className="product-form" noValidate onSubmit={onSubmit}>
      ...
      <div className="field-group"><label htmlFor="product-name">Ürün adı (zorunlu)</label>
        <input id="product-name" name="name" required disabled={pending} aria-invalid={Boolean(fieldErrors.name)} aria-describedby={describedBy('name')} />
        {fieldErrors.name && <p className="field-error" id="product-name-error">{fieldErrors.name}</p>}</div>
      <div className="product-form-pair">
        <div className="field-group"><label htmlFor="product-sku">SKU (isteğe bağlı)</label><input id="product-sku" name="sku" disabled={pending} /></div>
        <div className="field-group"><label htmlFor="product-brand">Marka (isteğe bağlı)</label><input id="product-brand" name="brand" disabled={pending} /></div>
        <div className="field-group"><label htmlFor="product-category">Kategori (isteğe bağlı)</label><input id="product-category" name="category" disabled={pending} /></div>
        <div className="field-group"><label htmlFor="product-model">Model (isteğe bağlı)</label><input id="product-model" name="model" disabled={pending} /></div>
        <div className="field-group"><label htmlFor="product-unit">Birim (isteğe bağlı)</label><input id="product-unit" name="unit" disabled={pending} /></div>
        <div className="field-group"><label htmlFor="product-reference-price">Referans fiyat (isteğe bağlı)</label>
          <input id="product-reference-price" name="referencePrice" type="number" min="0" step="0.01" inputMode="decimal" disabled={pending}
      ...
      <div className="form-actions"><button className="secondary-button" type="button" onClick={onCancel} disabled={pending}>İptal</button>
        <button className="primary-button compact-button" type="submit" disabled={pending}>{pending ? 'Oluşturuluyor…' : 'Ürün oluştur'}</button></div>
    </form>
  </main>;
}

function serverFieldErrors(error: ApiError): ProductFieldErrors {
...
export function ProductCreateScreen({ onCancel, onCreated, create = createProduct }: {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { onCreated(await create(input)); }
      ...
      const nextFieldErrors = serverFieldErrors(apiError); setError(apiError.message); setFieldErrors(nextFieldErrors);
