...
| **Staging contract (environment, DB runbooks, gates)** | [staging-contract.md](docs/operations/staging-contract.md), [staging-database-runbook.md](docs/operations/staging-database-runbook.md), [staging-acceptance.md](docs/operations/staging-acceptance.md) |
...
The migration runner applies the immutable 001–033 files (latest: `033_backup_worker_runtime`) for the ledger, authentication, Product Delivery tracer, People profiles/audits, Customer/Contact CRM, Product catalog, JobCard workspace notes/indexes/lifecycle timestamp constraints, Structured Sales Meeting details, entity-delete audit, realtime events, in-app notifications, job action locations, web push, engagement kinds, reverse geocoding, calendar, messaging, operational note contexts, linked follow-up cards, staff confidential notes, the messaging participant lifecycle, and the BR1–BR5 backup runtime contracts.
...
| `HEALTH_SCHEMA_VERSION` | production | exact `schema_migrations.version` for readiness (current: `033_backup_worker_runtime`; must equal the latest canonical migration in the deployed release); optional in development/test |
