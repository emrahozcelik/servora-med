...
describe.skipIf(!databaseUrl)('Sales Meeting PostgreSQL migrations', () => {
  it('runs clean migrations, preserves exact vocabularies and does not reapply', async () => {
    await withIsolatedDatabase(async (pool, store) => {
      const firstRun = await runMigrations({
        migrationsDirectory: MIGRATIONS_DIRECTORY,
        store,
      });
      expect(firstRun.appliedVersions).toHaveLength(40);
      expect(firstRun.appliedVersions.at(-1)).toBe('040_demo_lifecycle_simplification');
    ...
    await withIsolatedDatabase(async (pool, store) => {
      ...
      expect(upgrade).toEqual({
        appliedVersions: [
          ...
          '020_job_card_transition_note_contexts',
          '021_job_card_note_added_notification_kind',
          '022_job_card_follow_up_links',
