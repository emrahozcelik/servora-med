...
describe('SD2 catalog source', () => {
  it('derives expected head from catalog as 053', async () => {
...
describe('SD2 startup fail-fast', () => {
  ...
  it('BEHIND through 036 refused', async () => {
    ...
    expect(logs[0]?.msg).toMatch(/Run npm run migrate/);
      expect(logs[0]?.fields.pendingVersions).toEqual(['037_staff_offboarding_audit', '038_demo_dataset_audit_types', '039_contact_deleted_audit', '040_demo_lifecycle_simplification', '041_user_lifecycle_reconciliation', '042_unsuccessful_visit_reason', '043_job_card_schedule_and_assignment_history', '044_job_card_accountability_facts', '045_calendar_request_hash', '046_notification_state_realtime', '047_job_card_overdue_incidents', '048_overdue_episode_activation_legacy_first', '049_job_card_lifecycle_intents', '050_overdue_incident_scanner_source', '051_weekly_report_foundation', '052_weekly_report_recurrence', '053_overdue_submission_reminders']);
  });
  it('BEHIND many (029 incident) refused with 10 pending', async () => {
    const catalog = await loadMigrationCatalog(migrationsDirectory);
  ...
  it('AHEAD with 054_future refused', async () => {
    const catalog = await loadMigrationCatalog(migrationsDirectory);
    const pool = mockPool([...catalog.entries.map((e) => e.version), '054_future']);
    const { logger, logs } = createMockLogger();
...
describe('SD2 readiness matrix', () => {
  ...
  it('AHEAD → 503', async () => {
    const catalog = await loadMigrationCatalog(migrationsDirectory);
    const r = await healthStatusFor([...catalog.entries.map((e) => e.version), '054_future']);
    expect(r.statusCode).toBe(503);
  });
...
describe.skipIf(!process.env.TEST_DATABASE_URL)('SD2 disposable postgres acceptance', () => {
  ...
  it('E: AHEAD synthetic 054_future refused', async () => {
    await withSchema(async (pool) => {
      const catalog = await loadMigrationCatalog(migrationsDirectory);
      const store = new PostgresMigrationStore(pool);
      await runMigrations({ migrationsDirectory, store });
      await pool.query("INSERT INTO schema_migrations (version) VALUES ('054_future')");
