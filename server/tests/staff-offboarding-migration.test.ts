...
describe.skipIf(!databaseUrl)('R4A audit vocabulary migration', () => {
  it('freshly applies the full chain, preserves the audit allowlist, and adds the D4 audit vocabulary', async () => {
    await withIsolatedDatabase(async (pool) => {
      ...
      expect(result.appliedVersions).toHaveLength(43);
      expect(result.appliedVersions.at(-1)).toBe('043_job_card_schedule_and_assignment_history');
  ...
  it('upgrades a pre-037 database without rewriting existing audit rows', async () => {
    await withIsolatedDatabase(async (pool) => {
      ...
      expect(upgrade).toEqual({ appliedVersions: ['037_staff_offboarding_audit', '038_demo_dataset_audit_types', '039_contact_deleted_audit', '040_demo_lifecycle_simplification', '041_user_lifecycle_reconciliation', '042_unsuccessful_visit_reason', '043_job_card_schedule_and_assignment_history'] });
