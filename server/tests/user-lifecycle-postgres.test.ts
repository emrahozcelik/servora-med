...
describe.skipIf(!databaseUrl)('User/Staff lifecycle PostgreSQL acceptance', () => {
  it('runs 0→043 and permanently deletes a pristine Staff atomically', async () => {
    await withIsolatedDatabase(async (pool, store) => {
      ...
      expect(migrationResult.appliedVersions).toHaveLength(47);
      expect(migrationResult.appliedVersions.at(-1)).toBe('047_job_card_overdue_incidents');
  ...
  it('upgrades 040→043 without changing existing business rows', async () => {
    ...
    await withIsolatedDatabase(async (pool, store) => {
      ...
      expect(upgrade.appliedVersions).toEqual(['041_user_lifecycle_reconciliation', '042_unsuccessful_visit_reason', '043_job_card_schedule_and_assignment_history', '044_job_card_accountability_facts', '045_calendar_request_hash', '046_notification_state_realtime', '047_job_card_overdue_incidents']);
