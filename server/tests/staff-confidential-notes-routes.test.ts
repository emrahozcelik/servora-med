...
const staffUser: CurrentUser = { ...admin, id: 'staff-1', role: 'STAFF' };
...
implements StaffConfidentialNotesRepository {
  ...
  async findSubject(organizationId: string, userId: string) {
    ...
    return found
      ? { id: found.id, organizationId: found.organizationId, role: found.role, isActive: found.isActive }
      : null;
  }
...
describe('Staff confidential notes routes', () => {
  it('creates a note with 201 for an admin', async () => {
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/api/staff/staff-1/confidential-notes',
  ...
  it('lists notes with 200 and paginated shape', async () => {
    ...
    repository.notes.push(
      { id: 'n-1', organizationId: 'org-1', staffUserId: 'staff-1', authorUserId: 'admin-1', body: 'bir', createdAt: '2026-08-03T10:00:00.000Z' },
      { id: 'n-2', organizationId: 'org-1', staffUserId: 'staff-1', authorUserId: 'admin-1', body: 'iki', createdAt: '2026-08-03T10:00:01.000Z' },
    );
    const app = await buildNotesApp(repository, admin);
    const response = await app.inject({ method: 'GET', url: '/api/staff/staff-1/confidential-notes?limit=1&offset=1' });
    expect(response.statusCode).toBe(200);
    expect(response.json()).toEqual({
      items: [{ id: 'n-1', organizationId: 'org-1', staffUserId: 'staff-1', authorUserId: 'admin-1', authorName: 'Admin', body: 'bir', createdAt: '2026-08-03T10:00:00.000Z' }],
      total: 2,
      limit: 1,
      offset: 1,
    });
  ...
  it('rejects unknown list query parameters with 400', async () => {
    const repository = new MemoryStaffConfidentialNotesRepository();
    const app = await buildNotesApp(repository, admin);
    const response = await app.inject({ method: 'GET', url: '/api/staff/staff-1/confidential-notes?before=2026-01-01' });
    expect(response.statusCode).toBe(400);
  });
  ...
  it('validates limit and offset bounds', async () => {
    ...
    for (const query of ['limit=0', 'limit=101', 'limit=-1', 'limit=abc', 'offset=-2', 'offset=x']) {
      const response = await app.inject({ method: 'GET', url: `/api/staff/staff-1/confidential-notes?${query}` });
  ...
  it('denies staff actors with 403 before any subject lookup', async () => {
    ...
    const list = await app.inject({ method: 'GET', url: '/api/staff/staff-1/confidential-notes' });
  ...
  it('returns 404 for a subject outside the actor organization', async () => {
    const repository = new MemoryStaffConfidentialNotesRepository();
    const crossOrgAdmin: CurrentUser = { ...admin, id: 'admin-x', organizationId: 'org-2' };
