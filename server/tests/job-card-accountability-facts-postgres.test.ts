...
describe.skipIf(!databaseUrl)('FOUNDATION-2 accountability facts on real PostgreSQL', () => {
  it('migration 044 creates an empty fact table on top of 043 data', async () => {
    ...
    try {
      ...
      expect(applied.appliedVersions).toEqual(['044_job_card_accountability_facts', '045_calendar_request_hash', '046_notification_state_realtime', '047_job_card_overdue_incidents', '048_overdue_episode_activation_legacy_first', '049_job_card_lifecycle_intents', '050_overdue_incident_scanner_source', '051_weekly_report_foundation']);
  ...
  it('restore validation accepts a 046 database with consistent facts', async () => {
    ...
    try {
      ...
      const manifest = {
        database: { schemaVersion: '051_weekly_report_foundation' },
      } as unknown as RestoreManifestV1;
      const evidence = await validateRestoredDatabase(url.toString(), manifest);
      expect(evidence.schemaVersion).toBe('051_weekly_report_foundation');
