...
describe.skipIf(!databaseUrl)('normal customer scheduling PostgreSQL contract', () => {
  ...
  it('NJS-14: start-only patch on a NULL/NULL delivery persists the canonical end with history', async () => {
    const adminPool = new Pool({ connectionString: databaseUrl });
    const schema = `normal_sched_${randomUUID().replaceAll('-', '')}`;
    let pool: Pool | null = null;
    try {
      ...
      const organizationId = (await pool.query<{ id: string }>(
        `INSERT INTO organizations (name, timezone)
         VALUES ('Normal scheduling compat', 'Europe/Istanbul') RETURNING id`,
      )).rows[0]!.id;
      const managerId = await insertUser(pool, organizationId, 'MANAGER', 'Manager');
      const staffId = await insertUser(pool, organizationId, 'STAFF', 'Staff');
      const staff2Id = await insertUser(pool, organizationId, 'STAFF', 'Staff Two');
      const customerId = (await pool.query<{ id: string }>(
        `INSERT INTO customers (organization_id, name, customer_type, status)
         VALUES ($1, 'Dünya Klinik', 'clinic', 'active') RETURNING id`,
        [organizationId],
      ...
      // Legacy/demo-shaped row: interval job without any persisted schedule.
      const jobId = (await pool.query<{ id: string }>(
        `INSERT INTO job_cards
           (organization_id, type, status, title, customer_id, assigned_to, created_by)
         VALUES ($1, 'PRODUCT_DELIVERY', 'NEW', 'Zamanı olmayan teslim', $2, $3, $4)
         RETURNING id`,
      ...
      await pool.query(
        `INSERT INTO job_card_schedule_revisions
         ...
         VALUES ($1, $2, 1, NULL, NULL, NULL, 'Europe/Istanbul', 'CREATE', $3)`,
      ...
      // A conflicting manual event inside the derived 30m interval must now
      // fail the same repair with CALENDAR_CONFLICT instead of persisting.
      await pool.query(
        ...
        [organizationId, staff2Id, managerId],
      );
      const conflictJobId = (await pool.query<{ id: string }>(
        `INSERT INTO job_cards
           (organization_id, type, status, title, customer_id, assigned_to, created_by)
         VALUES ($1, 'PRODUCT_DELIVERY', 'NEW', 'Çakışacak teslim', $2, $3, $4)
         RETURNING id`,
        [organizationId, customerB, staff2Id, managerId],
      )).rows[0]!.id;
