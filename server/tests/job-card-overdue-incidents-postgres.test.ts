  ...
  it('history page returns incident DTOs in breached_at DESC order', async () => {
    await withSchema(async (pool) => {
      ...
      expect(page.items[0]).toEqual({
        ...
        // OVR-4 additive measurement fields: incident breach→recovery duration
        // (whole seconds), and no manual reminder exists in this scenario.
        totalDelaySeconds: Math.floor(
          (reservedStart.getTime() - Date.parse('2026-08-03T07:30:00.000Z')) / 1000,
        ),
