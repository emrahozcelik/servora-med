...
describe('loadMigrationCatalog', () => {
  ...
  it('proves real repository catalog: count 41, first 001_auth_foundation, last 041_user_lifecycle_reconciliation', async () => {
    ...
    expect(catalog.count).toBe(41);
    expect(catalog.entries[0]?.version).toBe('001_auth_foundation');
    expect(catalog.entries[0]?.number).toBe(1);
    expect(catalog.head?.version).toBe('041_user_lifecycle_reconciliation');
    expect(catalog.head?.number).toBe(41);
...
describe('compareMigrationState', () => {
  ...
  it('BEHIND models real incident: catalog 001..041 vs DB 001..029', async () => {
    const migrationsDirectory = fileURLToPath(new URL('../src/db/migrations', import.meta.url));
    const catalog = await loadMigrationCatalog(migrationsDirectory);
    ...
    if (result.status === 'BEHIND') {
      ...
      expect(result.expectedHead).toBe('041_user_lifecycle_reconciliation');
...
describe('compareMigrationState strict AHEAD (SD1 repair)', () => {
  ...
  it('full catalog + 042_alpha + 042_beta (duplicate number) → DIVERGED', async () => {
    const catalog = await loadMigrationCatalog(fileURLToPath(new URL('../src/db/migrations', import.meta.url)));
    const applied = [...catalog.entries.map((e) => e.version), '042_alpha', '042_beta'];
    const result = compareMigrationState(catalog, applied);
    expect(result.status).toBe('DIVERGED');
  ...
  it('full catalog + 042_future + 043_future → AHEAD', async () => {
    const catalog = await loadMigrationCatalog(fileURLToPath(new URL('../src/db/migrations', import.meta.url)));
    const applied = [...catalog.entries.map((e) => e.version), '042_future_a', '043_future_b'];
    const result = compareMigrationState(catalog, applied);
    expect(result.status).toBe('AHEAD');
    if (result.status === 'AHEAD') expect(result.unexpectedVersions).toEqual(['042_future_a', '043_future_b']);
  });
