...
describe('loadMigrationCatalog', () => {
  ...
  it('proves real repository catalog: count 37, first 001_auth_foundation, last 037_staff_offboarding_audit', async () => {
    ...
    expect(catalog.head?.number).toBe(37);
...
describe('compareMigrationState', () => {
  ...
  it('BEHIND models real incident: catalog 001..037 vs DB 001..029', async () => {
    const migrationsDirectory = fileURLToPath(new URL('../src/db/migrations', import.meta.url));
    const catalog = await loadMigrationCatalog(migrationsDirectory);
    ...
    if (result.status === 'BEHIND') {
      ...
      expect(result.expectedHead).toBe('037_staff_offboarding_audit');
