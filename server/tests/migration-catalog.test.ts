...
describe('loadMigrationCatalog', () => {
  ...
  it('proves real repository catalog: count 47, first 001_auth_foundation, last 047_job_card_overdue_incidents', async () => {
    ...
    expect(catalog.head?.number).toBe(47);
...
describe('compareMigrationState', () => {
  ...
  it('BEHIND models real incident: catalog 001..044 vs DB 001..029', async () => {
    ...
    if (result.status === 'BEHIND') {
      ...
      expect(result.expectedHead).toBe('047_job_card_overdue_incidents');
...
describe('compareMigrationState strict AHEAD (SD1 repair)', () => {
  ...
  it('full catalog + 048_alpha + 048_beta (duplicate number) → DIVERGED', async () => {
    const catalog = await loadMigrationCatalog(fileURLToPath(new URL('../src/db/migrations', import.meta.url)));
    const applied = [...catalog.entries.map((e) => e.version), '048_alpha', '048_beta'];
    const result = compareMigrationState(catalog, applied);
    expect(result.status).toBe('DIVERGED');
  ...
  it('full catalog + 048_future + 049_future → AHEAD', async () => {
    const catalog = await loadMigrationCatalog(fileURLToPath(new URL('../src/db/migrations', import.meta.url)));
    const applied = [...catalog.entries.map((e) => e.version), '048_future_a', '049_future_b'];
    const result = compareMigrationState(catalog, applied);
    expect(result.status).toBe('AHEAD');
    if (result.status === 'AHEAD') expect(result.unexpectedVersions).toEqual(['048_future_a', '049_future_b']);
  });
